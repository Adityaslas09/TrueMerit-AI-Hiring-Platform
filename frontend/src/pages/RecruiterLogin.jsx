...
const RecruiterLogin = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex bg-[#0A0D14] text-textLight relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute bottom-1/4 -right-[20%] w-[50%] h-[50%] bg-purple-500/10 rounded-full blur-[120px] pointer-events-none"></div>

      <div className="w-full max-w-md m-auto bg-[#1C212B]/80 backdrop-blur-3xl border border-purple-500/20 p-8 sm:p-10 rounded-3xl shadow-[0_0_40px_rgba(168,85,247,0.1)] relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
           ...
           <p className="text-gray-400 text-sm mt-1">Source the top 1% tech talent</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative group">
            <input 
              ...
              className="peer w-full bg-[#1C212B] border border-gray-700/50 rounded-xl px-4 pt-6 pb-2 text-white focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all font-medium text-sm"
              value={email} onChange={(e) => setEmail(e.target.value)}
            />
            <label htmlFor="email" className="absolute left-4 top-4 text-gray-500 text-sm transition-all pointer-events-none peer-placeholder-shown:top-3.5 peer-placeholder-shown:text-sm peer-focus:top-1.5 peer-focus:text-[10px] peer-focus:font-semibold peer-focus:text-purple-400 peer-valid:top-1.5 peer-valid:text-[10px] peer-valid:font-semibold peer-valid:text-gray-400">
              Work Email Address
            </label>
          </div>
          ...
          <button 
            type="submit" 
            className="w-full bg-purple-600 hover:bg-purple-500 text-white font-bold py-3.5 rounded-xl transition-all shadow-[0_0_20px_rgba(168,85,247,0.2)] mt-6 flex justify-center items-center gap-2 group active:scale-[0.98]"
          >
        ...
        <p className="text-center text-gray-400 mt-8 text-sm">
          Are you a candidate? <Link to="/login/student" className="text-blue-400 hover:text-blue-300 font-bold">Login here</Link>
        </p>
      </div>
    </div>
  );
